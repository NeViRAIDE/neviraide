...
---@field activeParameter? integer
---@field ft? string
---@field message NeviraideUIMessage
local M = {}
M.__index = M
...
function M.get_char(buf)
  ...
  local win = buf == vim.api.nvim_win_get_buf(current_win) and current_win
...
function M.on_signature(_, result, ctx, config)
  ...
  if not (result and result.signatures) then
    if not config.trigger then vim.notify('No signature help available') end
    return
  end
...
function M.new(help) return setmetatable(help, M) end

function M:active_parameter(sig_index)
  if
    self.activeSignature
    and self.signatures[self.activeSignature + 1]
    and sig_index ~= self.activeSignature + 1
  then
    return
  end
  ...
  if
    sig.activeParameter
    and sig.parameters
    and sig.parameters[sig.activeParameter + 1]
  then
  ...
  if
    ...
    and sig.parameters
    and sig.parameters[self.activeParameter + 1]
  then
...
function M:format_active_parameter(sig, param)
  ...
  if type(label) == 'string' then
    ...
    if from then
      self.message:append(NeviraideUIText('', {
...
function M:format_signature(sig_index, sig)
  ...
  self.message:append(
    NeviraideUIText.syntax(self.ft, self.message:height() - count)
  )
  local param = self:active_parameter(sig_index)
  if param then self:format_active_parameter(sig, param) end
  self.message:newline()
  ...
  local params = vim.tbl_filter(
    function(p) return p.documentation end,
    sig.parameters or {}
  )
  ...
  if #params > 0 then
    for _, p in ipairs(sig.parameters) do
      if p.documentation then
        local pdoc =
          table.concat(Format.format_markdown(p.documentation or ''), '\n')
        local line = { '-' }
        if p.label then
          local label = p.label
