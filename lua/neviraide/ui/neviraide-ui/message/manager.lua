...
---@type table<number, NeviraideUIMessage>
M._history = {}

---@type table<number, NeviraideUIMessage>
M._messages = {}

function M.tick() return _tick end
...
function M.sort(messages, reverse)
  table.sort(
    messages,
    ---@param a NeviraideUIMessage
    ---@param b NeviraideUIMessage
    function(a, b)
      local ret = (a.mtime == b.mtime) and (a.id < b.id) or (a.mtime < b.mtime)
      if reverse then ret = not ret end
      return ret
    end
  )
end

function M.get_by_id(id) return M._history[id] end

---@class NeviraideUIMessageOpts
---@field history? boolean
---@field sort? boolean
---@field reverse? boolean
---@field count? number
---@field messages? NeviraideUIMessage[]

---@param filter? NeviraideUIFilter
---@param opts? NeviraideUIMessageOpts
...
function M.get(filter, opts)
  ...
  for _, message in pairs(messages) do
    if not filter or message:is(filter) then table.insert(ret, message) end
  end
  if opts.sort then M.sort(ret, opts.reverse) end
...
---@param fn fun(message: NeviraideUIMessage)
---@param filter? NeviraideUIFilter
