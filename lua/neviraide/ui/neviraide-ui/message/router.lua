...
---@class NeviraideUIRoute
---@field view NeviraideUIView
---@field filter NeviraideUIFilter
---@field opts? NeviraideUIRouteOptions|NeviraideUIViewOptions

---@class NeviraideUIRouteOptions
---@field stop boolean
---@field skip boolean

---@class NeviraideUIRouteConfig
---@field view string
---@field filter NeviraideUIFilter
---@field opts? NeviraideUIRouteOptions|NeviraideUIViewOptions

local M = {}
---@type NeviraideUIRoute[]
M._routes = {}
M._tick = 0
...
---@param route NeviraideUIRouteConfig
---@param pos? number
...
function M.redirect(cmd, routes)
  ...
  if type(cmd) == 'string' then
    local cmd_str = cmd
    cmd = function() vim.cmd(cmd_str) end
  end
  ...
  M._routes = vim.tbl_filter(
    function(r) return not vim.tbl_contains(added, r) end,
    M._routes
  )
end
...
function M.view_stats()
  ...
  for view, _ in pairs(views) do
    if #view._messages > 0 then
      if not ret[view._opts.view] then ret[view._opts.view] = 0 end
...
function M.get_views()
  ---@type table<NeviraideUIView, boolean>
  local views = {}
  for _, route in ipairs(M._routes) do
    if route.view then views[route.view] = true end
  end
  return views
end
...
function M.update()
  if Util.is_exiting() then return end

  if M._updating then return end
  ...
  for view, _ in pairs(views) do
    ...
    if #view._messages ~= count then updates[view] = true end
  end
    ...
    for _, route in ipairs(M._routes) do
      if message:is(route.filter) then
        if not route.opts.skip then
          ...
          route.view._route_opts = vim.tbl_deep_extend(
            'force',
            route.view._route_opts or {},
            route.opts or {}
          )
          updates[route.view] = true
        end
        if route.opts.stop ~= false then break end
      end
    end
  end
...
function M.echo_pending()
  ...
  for _, message in ipairs(messages) do
    for _, line in ipairs(message._lines) do
      ---@param t NuiText
      local chunk = vim.tbl_map(
        function(t) return { t:content(), t.extmark.hl_group } end,
        line._texts
      )
