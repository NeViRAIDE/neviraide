...
local Highlight = require('neviraide.ui.neviraide-ui.text.highlight')
local Util = require('neviraide.ui.neviraide-ui.utils')
local NuiLine = require('nui.line')
local Object = require('nui.object')

---@alias NeviraideUIChunk { [0]: integer, [1]: string}
---@alias NeviraideUIContent string|NeviraideUIChunk|NuiLine|NuiText|NeviraideUIBlock

---@class NeviraideUIBlock
---@field _lines NuiLine[]
---@overload fun(content?: NeviraideUIContent|NeviraideUIContent[], highlight?: string|table): NeviraideUIBlock
...
function Block:init(content, highlight)
  self._lines = {}
  if content then self:append(content, highlight) end
end

function Block:clear() self._lines = {} end
...
function Block:height() return #self._lines end

function Block:is_empty() return #self._lines == 0 end
...
function Block:_fix_extmarks()
  for _, line in ipairs(self._lines) do
    for _, text in ipairs(line._texts) do
      if text.extmark then text.extmark.id = nil end
    end
  end
end
...
function Block:render(bufnr, ns_id, linenr_start, linenr_end)
  ...
  for _, line in ipairs(self._lines) do
    ...
    linenr_start = linenr_start + 1
    if linenr_end then linenr_end = linenr_end + 1 end
  end
end
...
function Block:_append(content, highlight)
  if #self._lines == 0 then table.insert(self._lines, NuiLine()) end
...
---@param contents NeviraideUIContent|NeviraideUIContent[]
---@param highlight? string|table data for highlight
...
function Block:append(contents, highlight)
  ...
  if
    contents._texts
    or contents._content
    or contents._lines
    or type(contents[1]) == 'number'
  then
    contents = { contents }
  end

  ---@cast contents NeviraideUIContent[]
...
function Block:last_line() return self._lines[#self._lines] end
...
function Block:trim_empty_lines()
  ...
  while
    #self._lines > 0 and vim.trim(self._lines[#self._lines]:content()) == ''
  do
    table.remove(self._lines)
  end
end

function Block:newline() table.insert(self._lines, NuiLine()) end
