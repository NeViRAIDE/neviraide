...
local Treesitter = require('neviraide.ui.neviraide-ui.text.treesitter')
local Syntax = require('neviraide.ui.neviraide-ui.text.syntax')
local Markdown = require('neviraide.ui.neviraide-ui.text.markdown')

---@class NeviraideUIExtmark
---@field col? number
...
---@class NeviraideUIText: NuiText
---@field super NuiText
---@field on_render? fun(text: NeviraideUIText, buf:number, line: number, byte:number, col:number)
---@overload fun(content:string, highlight?:string|NeviraideUIExtmark):NeviraideUIText
---@diagnostic disable-next-line: undefined-field
local NeviraideUIText = NuiText:extend('NeviraideUIText')

function NeviraideUIText.virtual_text(text, hl_group)
  local content = (' '):rep(vim.api.nvim_strwidth(text))
  return NeviraideUIText(content, {
...
function NeviraideUIText.cursor(col)
  return NeviraideUIText(' ', {
    hl_group = 'NeviraideUICursor',
    col = col,
    relative = true,
  })
end
...
---@param col? number
function NeviraideUIText.syntax(lang, lines, col)
  return NeviraideUIText('', {
    lang = lang,
    col = col,
    lines = lines,
...
---@return nil
function NeviraideUIText:highlight(bufnr, ns_id, linenr, byte_start)
  if not self.extmark then return end

  if self.extmark.lang then
    local range = { linenr - self.extmark.lines, 0, linenr, byte_start + 1 }
    if self.extmark.col then range[2] = byte_start + self.extmark.col - 1 end
  ...
  if extmark.relative then
    ...
    if extmark.col then extmark.col = extmark.col + byte_start end
  ...
  NeviraideUIText.super.highlight(self, bufnr, ns_id, linenr, byte_start)
