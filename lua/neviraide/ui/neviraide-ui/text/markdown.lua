...
function M.is_code_block(line) return line and line:find('^%s*```') end

function M.is_empty(line) return line and line:find('^%s*$') end
...
function M.parse(text, opts)
  ...
  while l <= #lines do
    ...
    if M.is_empty(line) then
      ...
      if
        not (
          M.is_code_block(lines[l + 1])
          or M.is_rule(lines[l + 1])
          or is_start
          or is_end
        )
      then
    ...
    elseif M.is_rule(line) then
      ...
      local prev = ret[#ret]
      if prev and prev.code then table.insert(ret, { line = '' }) end
...
function M.format(message, text, opts)
  ...
  local function emit_md()
    if md_lines > 0 then
      message:append(NeviraideUIText.syntax('markdown', md_lines))
      md_lines = 0
    end
  end
    ...
    if block.code then
      ...
      for c, line in ipairs(block.code) do
        ...
        if c == #block.code then
          message:append(NeviraideUIText.syntax(block.lang, #block.code))
        else
          message:newline()
        end
      end
...
function M.keys(buf)
  if vim.b[buf].markdown_keys then return end
  ...
  local function map(lhs)
    vim.keymap.set('n', lhs, function()
      ...
      for pattern, handler in pairs({
        ...
        ['%[.-%]%((%S-)%)'] = require('neviraide.ui.neviraide-ui.utils').open, -- markdown links
      }) do
        local from = 1
        local to, url
        while from do
