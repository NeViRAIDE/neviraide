...
function M.get_query(lang)
  if not M.queries[lang] then
    ...
    M.queries[lang] = (
      vim.treesitter.query.get or vim.treesitter.query.get_query
    )(lang, 'highlights')
  end
...
function M.get_lang(ft)
  return vim.treesitter.language.get_lang
...
function M.highlight(buf, ns, range, lang)
  ...
  parser:for_each_tree(function(tstree, tree)
    ...
    local iter =
      highlighter_query:iter_captures(tstree:root(), buf, range[1], range[3])
