...
local State = require('neviraide.ui.neviraide-ui.ui.state')
...
function M.is_error(kind)
  return vim.tbl_contains(
    { M.kinds.echoerr, M.kinds.lua_error, M.kinds.rpc_error, M.kinds.emsg },
    kind
  )
end

function M.is_warning(kind) return kind == M.kinds.wmsg end

function M.get(event, kind)
  local id = event .. '.' .. (kind or '')
  if not M._messages[id] then M._messages[id] = Message(event, kind) end
  return M._messages[id]
end

---@param kind MsgKind
---@param content NeviraideUIContent[]
function M.on_show(event, kind, content, replace_last)
  ...
  if State.skip(event, kind, content, replace_last) then return end
...
function M.on_showmode(event, content)
  if State.skip(event, content) then return end
  local message = M.get(event)
  if vim.tbl_isempty(content) then
    if event == 'msg_showmode' then Manager.remove(message) end
  else
    message:set(content)
    Manager.add(message)
  end
...
function M.on_return_prompt() return vim.api.nvim_input('<cr>') end

---@param content NeviraideUIChunk[]
function M.on_confirm(event, kind, content)
  if State.skip(event, kind, content) then return end
  local message = Message(event, kind, content)
  if not message:content():find('%s+$') then message:append(' ') end
  message:append(' ', 'NeviraideUICursor')
  Manager.add(message)
  vim.schedule(function() Manager.remove(message) end)
end

---@param entries { [1]: string, [2]: NeviraideUIChunk[]}[]
