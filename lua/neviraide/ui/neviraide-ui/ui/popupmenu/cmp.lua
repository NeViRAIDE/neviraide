...
local Popupmenu = require('neviraide.ui.neviraide-ui.ui.popupmenu')

local cmp = require('cmp')
local cmp_config = require('cmp.config')

---@class NeviraideUICmpSource: cmp.Source
...
function source:complete(_params, callback)
  if not Popupmenu.state.visible then return callback() end
...
function M.on_show()
  ...
  config.sources = cmp.config.sources({ { name = 'neviraide_ui_popupmenu' } })
  cmp.core:prepare()
  cmp.complete({
    config = config,
  })
...
function M.on_select() M.on_show() end
