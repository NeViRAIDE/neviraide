...
function M.format_abbr(item, prefix)
  ...
  if prefix and text:lower():find(prefix:lower(), 1, true) == 1 then
    item.text:append(text:sub(1, #prefix), 'NeviraideUIPopupmenuMatch')
    item.text:append(text:sub(#prefix + 1), 'NeviraideUICompletionItemWord')
  else
    item.text:append(text, 'NeviraideUICompletionItemWord')
  end
end
...
function M.format_menu(item)
  if item.menu and item.menu ~= '' then
    item.text:append(' ')
    item.text:append(item.menu, 'NeviraideUICompletionItemMenu')
  end
end
...
function M.format_kind(item)
  if item.kind and item.kind ~= '' then
    ...
    hl_group = Highlights.defaults[hl_group] and ('NeviraideUI' .. hl_group)
      or 'NeviraideUICompletionItemKindDefault'
    local icon = Config.options.popupmenu.kind_icons[item.kind]
    item.text:append(' ')
    if icon then item.text:append(vim.trim(icon) .. ' ', hl_group) end
...
function M.opts(state)
  ...
  local view = require('neviraide.ui.neviraide-ui.config.views').get_options(
    is_cmdline and 'cmdline_popupmenu' or 'popupmenu'
  )
  ...
  local padding = opts.border and opts.border.padding
    or {
      left = 0,
      right = 0,
      top = 0,
  ...
  if opts.border and opts.border.padding then
    opts.border.padding = vim.tbl_deep_extend(
      'force',
      {},
      padding,
      { left = 0, right = 0 }
...
function M.show(state)
  ...
  for _, item in ipairs(state.items) do
    if type(item) == 'string' then item = { word = item } end
    item.text = NuiLine()
    if padding.left then item.text:append(string.rep(' ', padding.left)) end
  end
  ...
  for _, item in ipairs(state.items) do
    if padding.right then item.text:append(string.rep(' ', padding.right)) end
  ...
  local items = vim.tbl_map(
    function(item) return Menu.item(item) end,
    state.items
  )
  ...
  if Util.is_blocking() then Util.redraw() end
...
function M.create(items, opts)
  ...
  if M.menu.border then Util.tag(M.menu.border.bufnr, 'popupmenu.border') end
...
---@param state Popupmenu
function M.on_show(state) M.show(state) end
...
function M.on_hide()
  Util.protect(function()
    ...
    if M.scroll then
      ...
      if M.menu then M.menu._.loading = false end
    end,
    retry_on_E11 = true,
    retry_on_E565 = true,
  })()
