...
function M.reset()
  M.reset = Util.debounce(200, function() M._errors = 0 end)
  M.reset()
end
...
function M.protect(fn, opts)
  if not fn then
    ...
    Util.panic(
      'nil passed to neviraide-ui.utils.call.protect. This should not happen.\n%s',
      trace
    )
  end
...
function M:on_error(err)
  ...
  if M._errors > M._max_errors then
    Util.panic('Too many errors. Disabling NeviraideUI')
  end
  M.reset()

  if self._opts.catch then pcall(self._opts.catch, err) end

  if err then
    if
      self._opts.ignore_keyboard_interrupt
      and err:lower():find('keyboard interrupt')
    then
      M._errors = M._errors - 1
      return
    end
    ...
    if
      not self._retry
      and err:find('Vim:E%d+')
      and self._opts.retry_on_vim_errors
    then
      self._retry = true
      return
    end
...
function M:format(err, stack)
  ...
  if stack then
    if self._opts.msg then table.insert(lines, err) end
...
function M:__call(...)
  ...
  local wrapped = function() return self._fn(vim.F.unpack_len(args)) end
  ...
  if self._opts.finally then pcall(self._opts.finally) end

  if not ok and self._defer_retry then
    vim.defer_fn(function() self(vim.F.unpack_len(args)) end, 100)
  end
