...
function M.reset_augroup()
  M.group = vim.api.nvim_create_augroup(
    'neviraide.ui.neviraide-ui.hacks',
    { clear = true }
  )
end
...
function M.fix_vim_sleuth() vim.g.sleuth_neviraide_ui_heuristics = 0 end
...
function M.fix_nohlsearch()
  M.fix_nohlsearch = Util.interval(30, function()
    if vim.o.hlsearch and vim.v.hlsearch == 0 then
      local m = require('neviraide.ui.neviraide-ui.ui.msg').get(
        'msg_show',
        'search_count'
      )
      require('neviraide.ui.neviraide-ui.message.manager').remove(m)
    end
  end, {
    enabled = function() return vim.o.hlsearch and vim.v.hlsearch == 1 end,
  })
  M.fix_nohlsearch()
end
...
function M.draw_cursor()
  require('neviraide.ui.neviraide-ui.utils.ffi').setcursor_mayforce(true)
end
...
function M.fix_redraw()
  ...
  local function wrap(fn, ...)
    ...
    if not inside_redraw then M.inside_redraw = false end
  ...
  M.on_module('cmp.utils.api', function(api)
    ...
    api.get_cursor = function()
      if api.is_cmdline_mode() then
        ...
        if pos then return { pos.bufpos.row, vim.fn.getcmdpos() - 1 } end
      end
      return get_cursor()
    end
...
function M.cmdline_force_redraw()
  if not require('neviraide.ui.neviraide-ui.utils.ffi').cmdpreview then
    return
  end
...
function M.hide_cursor()
  ...
  vim.schedule(function()
    if M._guicursor then vim.go.guicursor = 'a:NeviraideUIHiddenCursor' end
  end)
...
function M.on_module(module, fn)
  if package.loaded[module] then return fn(package.loaded[module]) end
