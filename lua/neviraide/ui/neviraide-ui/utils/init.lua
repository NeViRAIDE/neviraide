...
local Hacks = require('neviraide.ui.neviraide-ui.utils.hacks')
local Config = require('neviraide.ui.neviraide-ui.config')

local M = {}

M.stats = require('neviraide.ui.neviraide-ui.utils.stats')
M.call = require('neviraide.ui.neviraide-ui.utils.call')
M.nui = require('neviraide.ui.neviraide-ui.utils.nui')
...
function M.tag(buf, tag)
  ...
  if Config.options.debug and vim.api.nvim_buf_get_name(buf) == '' then
    local path = 'neviraide-ui://' .. buf .. '/' .. tag
    local params = {}
    if ft ~= '' and ft ~= 'neviraide-ui' then
...
function M.interval(ms, fn, opts)
  ...
  function T.keep()
    if M.is_exiting() then return false end
    return opts.enabled == nil or opts.enabled()
  end

  function T.running() return timer and not timer:is_closing() end

  function T.stop()
    if timer and T.running() then timer:stop() end
  end
  ...
  function T.__call()
    if not T.running() and T.keep() then timer = vim.defer_fn(T.fn, ms) end
  end
...
function M.diff_keys(a, b)
  ...
  for k, _ in pairs(a) do
    if not vim.deep_equal(a[k], b[k]) then diff[k] = true end
  end
...
function M.module_exists(mod) return pcall(_G.require, mod) == true end
...
function M.is_blocking(opts)
  ...
  for _, m in ipairs({ 'ic', 'ix', 'c', 'no', 'r%?', 'rm' }) do
    if mode.mode:find(m) == 1 then blocking_mode = true end
  end
  ...
  local reason = opts.blocking and mode.blocking and 'blocking'
    ...
    or #require('neviraide.ui.neviraide-ui.ui.cmdline').cmdlines > 0 and 'cmdline'
    or nil
...
function M.try(fn, ...) return M.call.protect(fn)(...) end
...
function M.warn_once(msg, ...) M.notify_once(msg, vim.log.levels.WARN, ...) end

function M.error_once(msg, ...) M.notify_once(msg, vim.log.levels.ERROR, ...) end

function M.warn(msg, ...) M.notify(msg, vim.log.levels.WARN, ...) end

function M.error(msg, ...) M.notify(msg, vim.log.levels.ERROR, ...) end

--- Will stop NeviraideUI and show error
function M.panic(msg, ...)
  require('neviraide.ui.neviraide-ui').disable()
  require('neviraide.ui.neviraide-ui.view.backend.notify').dismiss()
  vim.notify(msg:format(...), vim.log.levels.ERROR)
  error('NeviraideUI was stopped to prevent further errors', 0)
end

function M.info(msg, ...) M.notify(msg, vim.log.levels.INFO, ...) end

function M.debug(data)
  local file = './neviraide-ui.log'
...
M.settings = function(set)
  return require('neviraide.ui.neviraide-ui.utils.change_settings.' .. set)
end
