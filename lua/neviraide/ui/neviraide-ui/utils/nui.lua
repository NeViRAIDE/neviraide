...
---@param opts? NeviraideUINuiOptions
---@return _.NeviraideUINuiOptions
...
function M.normalize_win_options(opts)
  ...
  if opts.win_options and opts.win_options.winhighlight then
    opts.win_options.winhighlight =
      Util.nui.get_win_highlight(opts.win_options.winhighlight)
  end
end
...
function M.overlap(win1, win2)
  ...
  if x_right < x_left or y_bottom < y_top then return 0.0 end

  -- # The intersection of two axis-aligned bounding boxes is always an
  -- # axis-aligned bounding box
  local intersection_area = math.max(x_right - x_left, 1)
    * math.max(y_bottom - y_top, 1)
...
function M.normalize_popup_options(opts)
  ...
  if type(border) == 'string' then opts.border = { style = border } end

  -- border padding
  if opts.border then opts.border.padding = M.normalize_padding(opts.border) end

  -- fix border text
  if
    opts.border
    and (
      not opts.border.style
      or opts.border.style == 'none'
      or opts.border.style == 'shadow'
    )
  then
...
function M.normalize_padding(opts)
  opts = opts or {}
  if type(opts) == 'string' then opts = { style = opts } end
...
function M.win_buf_height(win)
  ...
  if not vim.wo[win].wrap then return vim.api.nvim_buf_line_count(buf) end
...
---@param _opts NeviraideUINuiOptions
---@return _.NeviraideUINuiOptions
function M.get_layout(dim, _opts)
  ---@type _.NeviraideUINuiOptions
