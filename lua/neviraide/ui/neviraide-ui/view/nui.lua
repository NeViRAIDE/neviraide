...
function NuiView:autohide()
  if self._opts.timeout then
    self._timer:start(self._opts.timeout, 0, function()
      if self._visible then vim.schedule(function() self:hide() end) end
      self._timer:stop()
    end)
  end
end
...
function NuiView:smart_move()
  if not Config.options.smart_move.enabled then return end
  if
    not (
      self._opts.type == 'popup'
      and self._opts.relative
      and self._opts.relative.type == 'editor'
    )
  then
    return
  end
  ...
  local nui_win = self._nui.border._.type == 'complex'
...
function NuiView:create()
  ...
  self._nui = self._opts.type == 'split' and require('nui.split')(opts)
    or require('nui.popup')(opts)
...
function NuiView:mount()
  ...
  if self._opts.close and self._opts.close.keys then
    self._nui:map(
      'n',
      self._opts.close.keys,
      function() self:hide() end,
      { remap = false, nowait = true }
    )
  end
end

---@param old NeviraideUINuiOptions
---@param new NeviraideUINuiOptions
function NuiView:reset(old, new)
  ...
  if self._nui then
    ...
    local diff = vim.tbl_filter(function(key)
      ...
      if key == 'timeout' then return false end
      return true
  ...
  if self._nui then
    ...
    Util.protect(function()
      if self._nui and not self._visible then
        ...
        if self._nui then self._nui._.loading = false end
      end,
      retry_on_E11 = true,
      retry_on_E565 = true,
    })()
...
function NuiView:get_layout()
  local layout = Util.nui.get_layout(
    ...
    then
      ...
      for _, m in ipairs(self._messages) do
        for _, l in ipairs(m._lines) do
          height = height
            + math.max(1, (math.ceil(l:width() / layout.size.width)))
        end
      end
      layout = Util.nui.get_layout(
...
function NuiView:fix_border()
  if
    ...
    and vim.api.nvim_win_is_valid(self._nui.border.winid)
  then
    local winhl = vim.api.nvim_win_get_option(
      self._nui.border.winid,
      'winhighlight'
    ) or ''
...
function NuiView:update_layout() self._nui:update_layout(self:get_layout()) end

function NuiView:is_mounted()
  if
    self._nui
    and self._nui.bufnr
    and not vim.api.nvim_buf_is_valid(self._nui.bufnr)
  then
    self._nui.bufnr = nil
  end
  ...
  if
    self._nui
    and self._nui.winid
    and not vim.api.nvim_win_is_valid(self._nui.winid)
  then
    self._nui.winid = nil
  end
...
function NuiView:show()
  ...
  if not self._nui then self:create() end

  if not self:is_mounted() then self:mount() end
  ...
  self._nui:show()
  if not self._nui.winid then return end
  self:tag()
